...
Add `swift-google-identity-accesscontextmanager-type` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-identity-accesscontextmanager-type.git --from 0.1.0-preview
```

Then add `GoogleIdentityAccessContextManagerType` to your target's dependencies:

```bash
swift package add-target-dependency GoogleIdentityAccessContextManagerType <target-name> --package swift-google-identity-accesscontextmanager-type
```
